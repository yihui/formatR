...
To install from [CRAN](http://cran.rstudio.com/package=formatR), use

```{r eval=FALSE}
install.packages('formatR')
```

To install the latest [development version](http://rforge.net/formatR), use
...
If the **shiny** packages has been installed, the function `tidy.app()` can launch a Shiny app to reformat R code like this ([live demo](https://yihui.shinyapps.io/formatR/)):

```r
formatR::tidy.app()
```

![R source code before tidying](http://i.imgur.com/CuEPthH.png)

After hitting the `Tidy` button:

![R source code after tidying](http://i.imgur.com/ulXmq9U.png)
...
We continue the example code in Section 2, using different arguments in `tidy.source()` such as `arrow`, `blank`, `indent`, `brace.newline` and `comment`, etc.
...
Blank lines are often used to separate complete chunks of R code, and arbitrary blank lines may cause failures in `tidy.source()` as well when the argument `blank = TRUE`, e.g.
